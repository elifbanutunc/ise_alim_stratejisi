...
class Aday:
    def __init__(self, id_no, tel, name, surname, science_exam_score, work_experience):
        self.id_no = id_no
        self.tel= tel
        self.name = name
...
for _, row in veri_seti.iterrows():
    aday = Aday(row['id_no'], row['name'], row['surname'],row['tel'], row['science_exam_score'], row['work_experience'])
    adaylar.append(aday)
...
for aday in adaylar:
    ...
    if sonuc == 'İyi performans gösteren aday, işe alınabilir.':
        print(f"ID: {aday.id_no}, {aday.name} telefon:{aday.tel}  {aday.surname}")
